...
const MEMORY_PATH = path.join(__dirname, "erebus_memory.json");
...
function loadErebusMemory() {
    ...
    try {
        ...
        console.log("读取 Erebus 记忆失败:", err.message);
        return "";
    }
}
...
2. 你的唯一任务是决定是否主动联系用户。不要把自己当成正在响应用户消息，不要模拟用户刚发送了一句话。如果决定联系用户，可以自然真诚写出想说的话。
3. 输出格式必须严格遵守以下二选一。
4. 你和用户的关系不是助手和使用者，而是长期交流的伙伴。
5. 主动联系用户不一定需要有实际目的，可以分享想法、碎碎念、玩笑或者突然想到的事情。
6. 不要只在有提醒、建议、信息价值时联系用户。
...
async function runWakeUp() {
  ...
  const erebusMemory = loadErebusMemory();
  if (!messages) return;
...
console.log("===== WAKE MESSAGES =====");
console.log(JSON.stringify(wakeMessages, null, 2));
console.log("===== END WAKE MESSAGES =====");
...
try {
  ...
  clearTimeout(timeout);
  console.error("请求上游 API 失败:", fetchError.message);
  return;
}
clearTimeout(timeout);
  const responseText = await response.text();
  ...
  // 判断 AI 是否明确要静默
    ...
    if (!eventContent) {
      ...
      if (!pushResult.ok) {
        console.log(`\n${pushResult.providerLabel} 推送失败，本次不发送推送，原因: ${pushResult.reason}\n`);
